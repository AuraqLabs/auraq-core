/**
 * @file cobe.texture.js
 * Exports:
 *   - default (base64 PNG data URI)
 *
 * @description World map texture for COBE v2, encoded as a base64 PNG data URI.
 * Land is white, ocean is black. Used by the globe fragment shader to determine
 * where to render land dots on the globe surface.
 *
 * Updated from v1 texture -- v2 uses a higher-quality map asset.
 *
 * Original map asset: https://en.wikipedia.org/wiki/File:World_map_blank_without_borders.svg
 * Original: https://github.com/shuding/cobe
 * License: MIT
 *
 * @module cobe/texture
 * @author Shu Ding (original) -- vendored and adapted by KinuCyber
 * @license GPL-3.0
 */

export default 'data:image/png;base64,'
